//lab8.cpp
//Edited by: Roy Banuelos
//Last Edit: May/19/2016
//
//
//CMPS 335
//Exam framework.
...
#define TESTER
              ...
              float tolnc=0, err;
              char outputErr[]={
                     "%7g%7g%7g%7g%13g%13g <- <<Error: Invalid Output, off by %g>>\n"
              };
              sscanf(argv[2],"%g", &tolnc);

              printf("%7s%7s%7s%7s%13s%13s\n",
              ...
              for (int i=0; i<81; i++){
                     sbsi.h = testDat[i*5];
                     sbsi.wc = testDat[i*5+1];
                     sbsi.bsa = testDat[i*5+2];
                     sbsi.vtc = testDat[i*5+3];

                     // pass arguments to function
                     double val = bodyShapeIndex(&sbsi);
                     err =abs(val- testDat[i*5+4]);  
                     if (err <= tolnc)
                            printf("%7g%7g%7g%7g%13g%13g\n",
                                   testDat[i*5],
                                   testDat[i*5+1],
                                   testDat[i*5+2],
                                   testDat[i*5+3],
